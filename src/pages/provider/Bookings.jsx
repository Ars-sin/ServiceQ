...
const TABS = ['pending','scheduled','active','completed','cancelled'].map(id => ({ id, label: id.charAt(0).toUpperCase() + id.slice(1) }))
...
export default function ProviderBookings() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
      ...
      <Tabs tabs={TABS.map(t => ({ ...t, count: bookings.filter(b => b.status === t.id).length }))} active={tab} onChange={setTab} />
