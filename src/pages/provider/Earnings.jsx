  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6 max-w-6xl mx-auto w-full pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <StatCard label="Gross Revenue"     value={formatPHP(grossRevenue)}   icon={TrendingUp}   color="success" />
        <StatCard label="Platform Fees"     value={formatPHP(platformFees)}   icon={DollarSign}   color="danger" />
        <StatCard label="Net Revenue"       value={formatPHP(netRevenue)}     icon={DollarSign}   color="brand" />
      ...
      {tab === 'transactions' && (
        <div className="card overflow-x-auto p-0 border border-gray-200/80 shadow-sm">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">Showing recent bookings</span>
          </div>
          {transactions.length === 0 ? (
            <div className="p-12 text-center text-gray-400">
              <p className="text-3xl mb-2">💰</p>
              <p className="font-semibold text-gray-700">No transaction records yet</p>
              <p className="text-xs text-gray-400 mt-1 max-w-sm mx-auto">
                Completed service bookings will appear here along with net earnings and payout release statuses.
              </p>
            </div>
          ) : (
